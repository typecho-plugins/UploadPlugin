...
class UploadPlugin_Action extends Typecho_Widget implements Widget_Interface_Do
{
    ...
    public function fix($plugName)
    {
        ...
        if($plugName){
            ...
            if (is_writable($dir)){
                chdir($dir);
                $plugNameNew=strtr($plugName, ['-master' => '']);
                if(is_dir($plugName)){
                    if(rename($plugName,$plugNameNew)){
                        $this->widget('Widget_Notice')->set(_t('成功修复插件：'.$plugName.'为'.$plugNameNew), NULL, 'success');
                    }else{
                        $this->widget('Widget_Notice')->set(_t('插件：'.$plugName.'修复失败'), NULL, 'error');
                    }
                }else{
                    $file=$plugName.'.php';
                    $fileNew=$plugNameNew.'.php';
                    if(rename($file,$fileNew)){
                        $this->widget('Widget_Notice')->set(_t('成功修复插件：'.$plugName.'为'.$plugNameNew.'.php'), NULL, 'success');
                    }else{
                        $this->widget('Widget_Notice')->set(_t('插件：'.$plugName.'修复失败'), NULL, 'error');
                    }
                 }
    ...
    {   
        $this->widget('Widget_User')->pass('administrator'); 
        $this->on($this->request->is('fix'))->fix($this->request->fix);
